...
namespace IBM.Watson.DeveloperCloud.Utilities
{
    ...
    static public class WaveFile
    {
        ...
        public static AudioClip ParseWAV(string clipName, byte[] data)
        {
            ...
            {
                Debug.LogError($"TextToSpeech: Malformed WAV header: {GetID(form.form_id)} != RIFF || {GetID(form.id)} != WAVE");
                return null;
            }
            ...
            while (reader.BaseStream.Position < reader.BaseStream.Length)
            {
                ...
                else if (GetID(chunk.id) == "data")
                {
                    if (!bHeaderFound)
                    {
                        Debug.LogError("TextToSpeech: Failed to find header.");
                        return null;
                    }
                    ...
                    Debug.Log($"TextToSpeech: WAV INFO, channels = {channels}, bps = {bps}," +
                        $"samples = {samples}, rate = {header.sample_rate}");
                    ...
                    {
                        Debug.LogError($"ParseWAV: Unspported BPS {bps.ToString()} in WAV data.");
                        return null;
                    }
        ...
        public static byte[] CreateWAV(AudioClip clip, int bps = 16)
        {
            ...
            {
                Debug.LogError($"CreateWAV: Unsupported BPS {bps.ToString()} in WAV data.");
                return null;
            }
